...
Instruction Compiler::compileWhileLoop(const Instruction &loopInstruction) {
  return Instruction(Instruction::OperationCode::WHILE,
                     loopInstruction.register1, loopInstruction.register2,
                     loopInstruction.register3, loopInstruction.block);
}
...
std::vector<Instruction> Compiler::instructionsOptimizations(
    std::vector<Instruction> &instructions) {
  ...
  for (size_t i = 0; i < instructions.size(); ++i) {
    ...
    if (instr.operationCode == Instruction::OperationCode::SAVE) {
      if (i + 1 < instructions.size() &&
          instructions[i + 1].operationCode ==
              Instruction::OperationCode::SAVE &&
    ...
    if (instr.operationCode == Instruction::OperationCode::ADD ||
      ...
      if (i + 1 < instructions.size() &&
          instructions[i + 1].operationCode == instr.operationCode &&
          instr.register1 == instructions[i + 1].register1 &&
    ...
    if ((instr.operationCode == Instruction::OperationCode::ADD ||
      ...
      try {
        ...
        optimizedInstructions.emplace_back(Instruction::OperationCode::SAVE,
                                           instr.register1, result);
        continue;
